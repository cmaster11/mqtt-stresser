...
func openConnection(uri *url.URL, tlsc *tls.Config, timeout time.Duration) (net.Conn, error) {
	...
	case "tcp":
		...
		if len(localAddr) > 0 && len(localInterface) > 0 {
			interf, err := net.InterfaceByName(localInterface)
			if err != nil {
				panic(err)
			}
			...
			//println(fmt.Sprintf("Got local addresses: %v", addrs))

			var foundAddrString string
			for _, addr := range addrs {
				addrString := addr.String()
				// Strip out last slash
				parts := strings.Split(addrString, "/")
				addrPure := parts[0]
				if addrPure == localAddr {
					foundAddrString = addrPure
					break
				}
			}
